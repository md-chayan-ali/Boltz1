...
class RegressionTester(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        checkpoint = os.path.expanduser('~/.boltz/boltz1.ckpt')
        if not os.path.expanduser(checkpoint):
    ...
    @pytest.mark.slow
    def test_boltz1_forward(self):
        exp_output = self.regression_feats
        feats = self.regression_feats["feats"]
        self.model_module.confidence_prediction = False

        seed_everything(self.regression_feats["seed"])
        act_output = self.model_module(
            feats=feats,
            recycling_steps=self.regression_feats["recycling_steps"],
            multiplicity_diffusion_train=self.regression_feats["multiplicity_diffusion_train"],
            return_intermediate=True,
        )

        check_keys = ["s", "z", "s_inputs", "relative_position_encoding"]
        for key in check_keys:
            exp_val = exp_output[key]
            act_val = act_output[key]
            assert exp_val.shape == act_val.shape, f"Shape mismatch in {key}"
            assert torch.allclose(exp_val, act_val, atol=1e-2)
            mean_abs_exp = torch.mean(torch.abs(exp_val))
            mean_diff = torch.mean(torch.abs(exp_val - act_val))
            assert mean_diff < 1e-3, f"Mean diff too large in {key}: {mean_diff}, mean_abs_exp: {mean_abs_exp}"
